 ...
 * Set ALLOWED_ORIGINS (comma-separated) for CORS configuration.
 */
...
const getAllowedOrigins = () => {
    ...
    const defaults = ['https://bluecoderhub.com', 'https://www.bluecoderhub.com'];
    
    // Add localhost for non-production
    if (process.env.VERCEL_ENV !== 'production') {
        defaults.push('http://localhost:5173', 'http://localhost:4173', 'http://localhost:3000');
    }
...
// Rate limit cleanup: Remove stale entries every 5 minutes
setInterval(() => {
    const now = Date.now();
...
export default async function handler(req, res) {
    // CORS - use dynamic origin list from environment
    ...
    try {
        // AbortController for request timeout (30 seconds)
